...
def enhancedFeatureExtractorDigit(datum):
    ...
    for n in range(10):
        features[(n + 1) * 10] = aspect < 0.69

    for n in range(5):
        features[-n] = nonzero > 300 and 1.0 or 0.0

    Above = float(aboveCenter) / nonzero
    for n in range(5):
        features[-(n + 1) * 10] = Above > 0.35 and 1.0 or 0.0

    Right = float(lastRight) / nonzero
    for n in range(1000, 1005):
        features[n] = Right < 0.27 and 1.0 or 0.0
    return features
...
def enhancedPacmanFeatures(state, action):
    ...
    for ghost in ghostStates:
      md = util.manhattanDistance(position, ghost.getPosition())
    ...
    foodDist = 100000
    for row in range (foods.width):
        for col in range(foods.height):
            if foods[row][col] == True:
                distance = util.manhattanDistance((row,col),position)
    ...
    features['ghostDist'] = nearestGhostDistance
    #features['score'] = score
    #features['capsules'] = len(capsules)
    #features['ghostDist'] = ghosts
    #features['foodCounter'] = 20 * food_count
    #features['ghosts'] = len(ghostStates)
    #print len(capsules)
    #features['capDist'] = nearestCapDist
    #features['ghostDist'] = 10 * nearestGhostDistance
    #rawScore = successor.getScore()
    #if rawScore != 0:
    #    rawScore = 1 / rawScore
    #features['rawScore'] = rawScore
