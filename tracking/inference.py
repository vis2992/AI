...
class ParticleFilter(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None: # for first case
            self.particles = [self.getJailPosition()] * self.numParticles
        else:
            allPossible, oldBelief = util.Counter(), self.getBeliefDistribution()
            for position in self.legalPositions:
                distance = util.manhattanDistance(position, pacmanPosition)
                allPossible[position] += emissionModel[distance] * oldBelief[position]
            if not any(allPossible.values()): # for second case
                self.initializeUniformly(gameState)
            else:
                list = []
                for _ in range(0, self.numParticles):
                    list.append(util.sample(allPossible))
                self.particles = list
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        particleList = list(itertools.product(self.legalPositions, repeat = self.numGhosts))
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        distribution = util.Counter()
        for part in self.particles: distribution[part] += 1
        distribution.normalize()
        return distribution
        #util.raiseNotDefined()
